...
/*
  ...
  Wrapped as an exported init fn for React. Accepts a container div,
  mounts the canvas inside it, and returns a cleanup function.
  =========================================
*/
...
export function initCarWorld(container: HTMLDivElement): () => void {
  ...
  // AbortController: one ac.abort() removes every listener we register
  ...
  const KIOSKS = [
    ...
    { id: 'p-markaba', label: 'Markaba AI',  color: 0xffb020 },
  ...
  window.addEventListener('themechange', (e) => applyTheme(e.detail.theme !== 'light'), sig);
  ...
  return function cleanup() {
    ...
    delete (window as any).CarControls;
  };
}
